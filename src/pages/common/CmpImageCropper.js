...
const EasyCrop = forwardRef((props, ref) => {
  ...
  const onMediaLoaded = useCallback(
    (mediaSize) => {
      const { srcwidth, srcheight } = mediaSize;
      const ratioWidth = srcheight * aspect;
    console.log('----------- cropper props---------', props)
    if(width && height){ 
        setCropSize({ width: width, height: height })
    }else{
        if (srcwidth > ratioWidth) {
            setCropSize({ width: ratioWidth, height: srcheight });
        } else {
            setCropSize({ width: srcwidth, height: srcwidth / aspect });
        }
    }
